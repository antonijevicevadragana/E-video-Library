                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('film.update', $film) }}" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-4">
                                <div
                                    class="input-group @error('running_h') is-invalid @enderror @error('running_m') is-invalid @enderror">
                                    <input type="text" aria-label="First name" class="form-control" id="running_h"
                                        name="running_h" value="{{ old('running_h', $film->running_h) }}">
                                    <input type="text" aria-label="Last name" class="form-control" id="running_m"
                                        name="running_m" value="{{ old('running_m', $film->running_m) }}">
                                </div>
                                @error('running_h')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                        ...
                        <div class="row mb-3 border border-start-0 border-top-0 border-end-0">
                            ...
                            <div class="col-10">
                                @foreach ($people as $p)
                                    <div
                                        class="form-check form-check-inline col-3 @error('directors') is-invalid @enderror">
                                        <input class="form-check-input" type="checkbox" name="directors[]"
                                            id="iChbDirectors{{ $p->id }}" value="{{ $p->id }}"
                                            @checked(in_array(
                                                    $p->id,
                                                    old(
                                                        'directors',
                                                        $film->directors()->pluck('id')->toArray())))>
                                        <label class="form-check-label"
                                            for="iChbDirectors{{ $p->id }}">{{ $p->full_name }}</label>
                                    </div>
                                @endforeach
                            ...
                            <div class="col-10">
                                @foreach ($people as $p)
                                    <div class="form-check form-check-inline col-3 @error('writers') is-invalid @enderror">
                                        <input class="form-check-input" type="checkbox" name="writers[]"
                                            id="iChbWriters{{ $p->id }}" value="{{ $p->id }}"
                                            @checked(in_array(
                                                    $p->id,
                                                    old(
                                                        'writers',
                                                        $film->writers()->pluck('id')->toArray())))>
                                        <label class="form-check-label"
                                            for="iChbWriters{{ $p->id }}">{{ $p->full_name }}</label>
                                    </div>
                                @endforeach
                                @error('writers')
                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="col-10">
                                @foreach ($people as $p)
                                    <div class="form-check form-check-inline col-3 @error('stars') is-invalid @enderror">
                                        <input class="form-check-input" type="checkbox" name="stars[]"
                                            id="iChbStars{{ $p->id }}" value="{{ $p->id }}"
                                            @checked(in_array(
                                                    $p->id,
                                                    old(
                                                        'stars',
                                                        $film->stars()->pluck('id')->toArray())))>
                                        <label class="form-check-label"
                                            for="iChbStars{{ $p->id }}">{{ $p->full_name }}</label>
                                    </div>
                                @endforeach
                                @error('stars')
                                    <span class="invalid-feedback" role="alert">
                                ...
                                @foreach ($genres as $g)
                                    <div class="form-check form-check-inline @error('genres') is-invalid @enderror">
                                        <input class="form-check-input" type="checkbox" name="genres[]"
                                            id="iChbGenres{{ $g->id }}" value="{{ $g->id }}"
                                            @checked(in_array(
                                                    $g->id,
                                                    old(
                                                        'genres',
                                                        $film->genres()->pluck('id')->toArray())))>
                                        <label class="form-check-label"
                                            for="iChbGenres{{ $g->id }}">{{ $g->name }}</label>
                                    </div>
                                @endforeach
                                @error('genre')
                                    <span class="invalid-feedback" role="alert">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-4 text-center">
                                <img id="showImage" src="{{ $film->imgSrc }}" alt="{{ $film->name }}" class="mb-2"
                                    style="width: 100px;" />
                            </div>
                            ...
                            <div class="col-6">
                                <div class="input-group col-6">
                                    <input type="file" class="form-control @error('image') is-invalid @enderror"
                                        id="image" name="image" value="{{ old('image', $film->image) }}"
                                        aria-describedby="btnRemoveImg">
                                    <button class="btn btn-outline-secondary" type="button"
                                        id="btnRemoveImg">{{ __('Remove') }}</button>
                                    @error('image')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
